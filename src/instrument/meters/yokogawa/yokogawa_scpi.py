...
class Wt300eScpi(ScpiInstrument):
    """
    ...
    def communicate_group(self, *names, **values):
        ...
        for key, value in values.items():
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_COMMUNICATE_SET.get(key), value)
        self.write(write_cmd)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_COMMUNICATE_GET.get(name))
    ...
    def display_group(self, *names, **values):
        ...
        for key, value in values.items():
            ...
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_DISPLAY_SET.get(key), func,
        ...
        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_DISPLAY_GET.get(name))
        return self.query(query_cmd)
    ...
    def harmonics_group(self, *names, **values):
        ...
        write_cmd = None
        for key, value in values.items():
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_HARMONIC_SET.get(key), value)
        self.write(write_cmd)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_HARMONIC_GET.get(name))
        return self.query(query_cmd)
    ...
    def input_voltage(self, *names, **values):
        ...
        write_cmd = None
        for key, value in values.items():
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_INPUT_VOLTAGE_SET.get(key), value)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_INPUT_VOLTAGE_GET.get(name))
        return self.query(query_cmd)
    ...
    def input_current(self, *names, **values):
        ...
        for key, value in values:
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_INPUT_CURRENT_SET.get(key), value)
        self.write(write_cmd)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_INPUT_CURRENT_GET.get(name))
        return self.query(query_cmd)
    ...
    def input_scaling(self, *names, **values):
        ...
        write_cmd = None
        for key, value in values:
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_INPUT_SCALING_SET.get(key), value)
        self.write(write_cmd)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_INPUT_SCALING_GET.get(name))
        return self.query(query_cmd)
    ...
    def input_filter(self, **values):
        ...
        write_cmd = None
        for key, value in values:
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_INPUT_FILTER_SET.get(key), value)
        self.write(write_cmd)
    ...
    def input_others(self, *names, **values):
        ...
        write_cmd = None
        for key, value in values:
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_INPUT_OTHERS_SET.get(key), value)
        self.write(write_cmd)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_INPUT_OTHERS_GET.get(name))
        return self.query(query_cmd)
    ...
    def integrate_group(self, *names, **values):
        ...
        if mode is not None:
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_INTEGRATE_SET.get('mode'), mode)
        timer = values.get('timer', None)
        if timer is not None:
            assert isinstance(timer, dict), 'timer must be a dict'
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_INTEGRATE_SET.get('timer'),
                                               timer.get('hour', 0), timer.get('minute', 0), timer.get('second', 0))
        state = values.get('state', None)
        if state is not None:
            if 'start' == state:
                write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_INTEGRATE.get('start'))
            elif 'stop' == state:
                write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_INTEGRATE.get('stop'))
            elif 'reset' == state:
                write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_INTEGRATE.get('reset'))
            else:
                self._logger.warn('the state "%s" expect "start", "stop" or "reset"', state)
        self.write(write_cmd)
        query_cmd = None
        if 'mode' in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_INTEGRATE.get('mode'))
        if 'timer' in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_INTEGRATE.get('timer'))
        if 'state' in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_INTEGRATE.get('state'))
        return self.query(query_cmd)
    ...
    #     """
    #     if mode is not None:
    #         self.write(Wt300eCmd.DICT_INTEGRATE_SET.get('mode'), mode)
    #     return self.query(Wt300eCmd.DICT_INTEGRATE.get('mode'))
    #
    ...
    #         self.write(self.write(Wt300eCmd.DICT_INTEGRATE_SET.get('timer'),
    #                                 0 if hour is None else hour,
    #                                 0 if minute is None else minute,
    #                                 0 if second is None else second))
    #     return self.query(Wt300eCmd.DICT_INTEGRATE.get('timer'))
    #
    # def start(self):
    #     """
    #     Starts integration.
    ...
    #         None
    #     """
    #     self.write(Wt300eCmd.DICT_INTEGRATE.get('start'))
    #
    # def stop(self):
    #     """
    #     Stops integration.
    ...
    #         None
    #     """
    #     self.write(Wt300eCmd.DICT_INTEGRATE.get('stop'))
    #
    # def reset(self):
    #     """
    ...
    #         None
    #     """
    #     self.write(Wt300eCmd.DICT_INTEGRATE.get('reset'))
    #
    # def state(self):
    #     """
    ...
    #     """
    #     return self.query(Wt300eCmd.DICT_INTEGRATE.get('state'))
    # end of integrate
    ...
    def measure_group(self, **values):
        ...
        write_cmd = None
        for key, value in values.items():
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_MEASURE_SET.get(key), value)
        self.write(write_cmd)
    ...
    def numeric_format(self, fmt=None):
        ...
        @return:
            格式化数据, 注意后面的换行字符'\n'
        """
        if Wt300eCmd.REGEX_NUMERIC_FORMAT.match(fmt) is not None:
            self.write(Wt300eCmd.DICT_NUMERIC_SET.get('fmt'), fmt)
        return self.query(Wt300eCmd.DICT_NUMERIC_GET.get('fmt'))
    ...
    def numeric_normal(self, *names, **values):
        ...
        write_cmd = None
        for key, value in values.items():
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_NUMERIC_NORMAL_SET.get(key), value)
        self.write(write_cmd)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_NUMERIC_NORMAL_GET.get(name))
        return self.query(query_cmd)
    ...
    def numeric_normal_value(self, nrf=1):
        ...
        return self.query(utils.contact_spci_cmd(Wt300eCmd.DICT_NUMERIC_NORMAL_GET.get('header').format(nrf),
    ...
    def numeric_list(self, *names, **values):
        ...
        write_cmd = None
        for key, value in values.items():
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_NUMERIC_LIST_SET.get(key), value)
        self.write(write_cmd)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_NUMERIC_LIST_GET.get(name))
        return self.query(query_cmd)
    ...
    def numeric_list_value(self, nrf=1):
        ...
        """
        return self.query(Wt300eCmd.DICT_NUMERIC_LIST_GET.get('value'), nrf)
    ...
    def numeric_hold(self, on_off=None):
        ...
        """
        if on_off in TUPLE_ON_OFF:
            self.write(Wt300eCmd.DICT_NUMERIC_SET.get('hold'), on_off)
        return self.query(Wt300eCmd.DICT_NUMERIC_GET.get('hold'))
    ...
    def rate_group(self, *names, **values):
        ...
        write_cmd = None
        for key, value in values.items():
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_RATE_SET.get(key), value)
        self.write(write_cmd)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_RATE_GET.get(name))
        return self.query(query_cmd)
    ...
    def status_group(self, *names, **values):
        ...
        write_cmd = None
        for key, value in values:
            write_cmd = utils.contact_spci_cmd(Wt300eCmd.DICT_STATUS_SET.get(key), value)
        self.write(write_cmd)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(Wt300eCmd.DICT_STATUS_GET.get(name))
        return self.query(query_cmd)
    ...
    def store_state(self, on_off):
        ...
        """
        if on_off in TUPLE_ON_OFF:
            self.write(Wt300eCmd.DICT_STORE_SET.get('state'), on_off)
        return self.query(Wt300eCmd.DICT_STORE_GET.get('state'))
    ...
    def store_interval(self, hour=None, minute=None, second=None):
        ...
        if hour is not None \
                or minute is not None \
                or second is not None:
            self.write(self.write(Wt300eCmd.DICT_STORE_SET.get('interval'),
                                  0 if hour is None else hour,
                                  0 if minute is None else minute,
                                  0 if second is None else second))
        return self.query(Wt300eCmd.DICT_STORE_GET.get('interval'))
    ...
    def store_panel(self, nrf=1):
        ...
        @return:
            None
        """
        self.write(Wt300eCmd.DICT_STORE_SET.get('panel'), nrf)
    ...
    def system_group(self, *names, **values):
        ...
        write_cmd = None
        for key, value in values.items():
            write_cmd = utils.contact_spci_cmd(write_cmd, Wt300eCmd.DICT_SYSTEM_SET.get(key), value)
        self.write(write_cmd)

        query_cmd = None
        for name in names:
            query_cmd = utils.contact_spci_cmd(query_cmd, Wt300eCmd.DICT_SYSTEM_GET.get(name))
        return self.query(query_cmd)
